...
function getLexingResult(input: string): chevrotain.ILexingResult {
...
class FormulaParser extends CstParser {
  ...
  SourceDeclaration = this.RULE("SourceDeclaration", () => {
    this.CONSUME(Source);
    this.CONSUME(Identifier);
    this.OPTION(() => this.CONSUME(Semicolon));
  });

  DimensionSpecifier = this.RULE("DimensionSpecifier", () => {
    this.OR1([
      {
        ALT: () => {
          this.OR3([
  ...
  DimensionDeclaration = this.RULE("DimensionDeclaration", () => {
  ...
  DimensionValue = this.RULE("DimensionValue", () => {
  ...
  ParenthisExpression = this.RULE("ParenthisExpression", () => {
    this.CONSUME(LeftParenthis);
    this.SUBRULE(this.Expression);
    this.CONSUME(RightParenthis);
  });
  ...
  OperandSimple = this.RULE("OperandSimple", () => {
    this.OR([
      {
        ALT: () => {
  ...
  OperandWithUnary = this.RULE("OperandWithUnary", () => {
  ...
  FilterRefference = this.RULE("FilterRefference", () => {
    this.CONSUME(Filter);
    this.CONSUME(Hash);
    this.CONSUME(Identifier);
  });
  IIFExpression = this.RULE("IIFExpression", () => {
    this.CONSUME(Iif);
    this.CONSUME(LeftParenthis);
    this.SUBRULE(this.Expression);
    this.OR([
...
export { getLexingResult, FormulaParser };
